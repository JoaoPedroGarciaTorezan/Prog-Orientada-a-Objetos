...
class Vinho:
    def __init__(self, codigo, nome, tipo, variedade, origem, preco):
        ...
        self.__nome = nome
        self.tipo = tipo #chama o setter (sem undeline)
    ...
    @variedade.setter #Validação da variedade do vinho
    ...
    @origem.setter #Validação da origem do vinho
...
class LimiteConsultaVinho(tk.Toplevel):
    def __init__(self, tipos, variedades, controle):
        ...
        self.comboboxTipo.bind("<<ComboboxSelected>>", self.ctrl.exibeTipo) #combobox gera um evento
    ...
    def exibeTipo(self,event):
        ...
        for vinho in self.listaVinhos:
            if vinho.tipo == tipoSel:
                self.limiteCons.textVinhos.insert(1.0, vinho.getVinho() + "\n\n") # para inserir -> self.limiteCons.textVinhos.insert
            self.limiteCons.textVinhos.config(state='disabled') 
