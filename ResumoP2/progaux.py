...
"""Sumário:
combobox retorna indice
any(not c.isalnum() and not c.isspace() for c in valor) - verifica se há caracteres especiais no valor
"""
...
class CtrlJogo:
    ...
    def consultaJogo(self):
        ...
        for jogo in self.jogos:
           for ava in jogo.avaliacoes:
            if (not f'{ava} estrelas' in self.listaAvaliacoes):
               self.listaAvaliacoes.append(f'{ava} estrelas')
    ...
    def avaliaHandler(self, event):
        ...
        #me retornando o indice do valor do combobox
    ...
    def exibeAvaliacao(self, event):
        ...
        for jogo in self.jogos:
            for avaliacao in set(jogo.avaliacoes):
                if f'{avaliacao} estrelas' == avaliSel:
